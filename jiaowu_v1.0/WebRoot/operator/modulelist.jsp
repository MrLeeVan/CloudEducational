...
<body>
								...
								<c:forEach items="${showPages.list}" var="module" varStatus="index">
									<tr>
										...
										<c:if test="${operator_session.qx_operatorshowthisModuleDetail || operator_session.qx_operatoreditmodule || operator_session.qx_operatordelthisModule }">
											...
											<%-- 需要的时候将注释去掉  不要改uri
											<c:if test="${operator_session.qx_operatorshowthisModuleDetail }">
												<a href="javascript:void(0);" onclick="showThisModuleDetail(${module.id})" >${_res.get("admin.common.see")}</a>&nbsp;|&nbsp;
											</c:if> --%>
    ...
    <script>
   	...
   	function showThisModuleDetail(id){
   		$.ajax({
   			url:"/operator/showthisModuleDetail",
   			data:{"id":id},
   			dataType:"json",
   			type:"post",
